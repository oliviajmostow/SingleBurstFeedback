...
def main():
        ...
        runs=['../m10_ic0_sg/sg_mass10-output','../sizetest_7/mb_mass10-output','../sizetest_8/mb_mass10-output','../sizetest_9/mb_mass10-output']
        plot_radius = 1/2*(radius[:-1] +radius[1:])
        density_sg = cd_function.calc_density(runs[0], spacing=spacing)
        density_7 = cd_function.calc_density(runs[1], spacing=spacing)
        density_8 = cd_function.calc_density(runs[2], spacing=spacing)
        density_9 = cd_function.calc_density(runs[3], spacing=spacing)
        df1, df2, df3, df4 = [pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame()]
        df1['radius'], df2['radius'], df3['radius'], df4['radius'] = [plot_radius, plot_radius, plot_radius, plot_radius]
        df1['density'], df2['density'], df3['density'], df4['density'] = [density_sg, density_7, density_8, density_9]
        df1['size'], df2['size'], df3['size'], df4['size'] = [0, 5, 2.5, 1]
        ...
        cmap.set_array([])
        densities = [density_7, density_8, density_9]
        sizes = [1e7,2.5e7,5e7]
        Z = [[0,0],[0,0]]
        levels = range(1, 6, 1)
        ...
        fig.colorbar(CS3, ax=ax).set_label('Mass Expelled Per Burst ($10^7$$M_{\odot}$)', size=16)
