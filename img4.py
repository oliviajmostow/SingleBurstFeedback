...
def main():
	snap = 60
	cat = DataLoader('../m8_dmo_hr/dmo_mass8-output', snap, 1, ['Masses','Coordinates','Potential','ParticleIDs'], fof_idx=0)
	x = cat['Coordinates'][:,0]
	y = cat['Coordinates'][:,1]
...
#	ax.plot(x_0, y_0, 'o')
	cat2 = DataLoader('../m8_dmo_hr/dmo_mass8-output', snap, 5, ['Coordinates'], fof_idx=0)
	...
	x_coord = inside[:,0]
	y_coord = inside[:,1]
	ax.hist2d(x_coord, y_coord, bins=200, norm=LogNorm())
	ax.plot(bh_x, bh_y, 'o')
