...
import pandas as pd
def main():
	cat = DataLoader('../hr_sg_mass10/hr_sg_mass10-output', 127, 1, ['Masses','Coordinates','SubhaloPos','SubhaloMass'], sub_idx=0)
...
#	density calculation from other file
	radius = np.logspace(-1.5,1,50)
	logr = np.log10(radius)
	radius_normal = np.logspace(-1.5,1,50)
...
#	ax.set_xlim(0,500)
	...
	fig, ax = plt.subplots()
	ax.plot(10**(1/2*(new_bins[1:]+new_bins[:-1])), average_slopes, color='blue')
	ax.set_xlabel('Radius (kpc)')
	ax.set_ylabel('\u03B1')
	ax.set_xscale('log')
	ax.set_ylim(-3, 1)
	ax.axvline(x=.076, color='black', ls='--', label='softening length')
	ax.axvline(x=(2.8*.076), color='gray', ls='--', label='2.8*softening')
	ax.legend()
	fig.savefig('slope2_sg10_smooth.pdf')
